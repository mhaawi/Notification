/* tslint:disable max-line-length */
import { TestBed, getTestBed } from '@angular/core/testing';
import { HttpClientTestingModule, HttpTestingController } from '@angular/common/http/testing';
import { HttpClient, HttpResponse } from '@angular/common/http';
import { of } from 'rxjs';
import { take, map } from 'rxjs/operators';
import * as moment from 'moment';
import { DATE_TIME_FORMAT } from 'app/shared/constants/input.constants';
import { SystemNotificationService } from 'app/entities/system-notification/system-notification.service';
import { ISystemNotification, SystemNotification, StatusType } from 'app/shared/model/system-notification.model';

describe('Service Tests', () => {
    describe('SystemNotification Service', () => {
        let injector: TestBed;
        let service: SystemNotificationService;
        let httpMock: HttpTestingController;
        let elemDefault: ISystemNotification;
        let currentDate: moment.Moment;
        beforeEach(() => {
            TestBed.configureTestingModule({
                imports: [HttpClientTestingModule]
            });
            injector = getTestBed();
            service = injector.get(SystemNotificationService);
            httpMock = injector.get(HttpTestingController);
            currentDate = moment();

            elemDefault = new SystemNotification(
                0,
                'AAAAAAA',
                StatusType.READ,
                currentDate,
                'AAAAAAA',
                'AAAAAAA',
                'AAAAAAA',
                'AAAAAAA',
                'AAAAAAA',
                'AAAAAAA',
                currentDate,
                false,
                currentDate
            );
        });

        describe('Service methods', async () => {
            it('should find an element', async () => {
                const returnedFromService = Object.assign(
                    {
                        sendDate: currentDate.format(DATE_TIME_FORMAT),
                        errorDate: currentDate.format(DATE_TIME_FORMAT),
                        deletedDate: currentDate.format(DATE_TIME_FORMAT)
                    },
                    elemDefault
                );
                service
                    .find(123)
                    .pipe(take(1))
                    .subscribe(resp => expect(resp).toMatchObject({ body: elemDefault }));

                const req = httpMock.expectOne({ method: 'GET' });
                req.flush(JSON.stringify(returnedFromService));
            });

            it('should create a SystemNotification', async () => {
                const returnedFromService = Object.assign(
                    {
                        id: 0,
                        sendDate: currentDate.format(DATE_TIME_FORMAT),
                        errorDate: currentDate.format(DATE_TIME_FORMAT),
                        deletedDate: currentDate.format(DATE_TIME_FORMAT)
                    },
                    elemDefault
                );
                const expected = Object.assign(
                    {
                        sendDate: currentDate,
                        errorDate: currentDate,
                        deletedDate: currentDate
                    },
                    returnedFromService
                );
                service
                    .create(new SystemNotification(null))
                    .pipe(take(1))
                    .subscribe(resp => expect(resp).toMatchObject({ body: expected }));
                const req = httpMock.expectOne({ method: 'POST' });
                req.flush(JSON.stringify(returnedFromService));
            });

            it('should update a SystemNotification', async () => {
                const returnedFromService = Object.assign(
                    {
                        content: 'BBBBBB',
                        status: 'BBBBBB',
                        sendDate: currentDate.format(DATE_TIME_FORMAT),
                        sourceId: 'BBBBBB',
                        sourceType: 'BBBBBB',
                        sourceName: 'BBBBBB',
                        applicationKey: 'BBBBBB',
                        error: 'BBBBBB',
                        errorShortName: 'BBBBBB',
                        errorDate: currentDate.format(DATE_TIME_FORMAT),
                        deleted: true,
                        deletedDate: currentDate.format(DATE_TIME_FORMAT)
                    },
                    elemDefault
                );

                const expected = Object.assign(
                    {
                        sendDate: currentDate,
                        errorDate: currentDate,
                        deletedDate: currentDate
                    },
                    returnedFromService
                );
                service
                    .update(expected)
                    .pipe(take(1))
                    .subscribe(resp => expect(resp).toMatchObject({ body: expected }));
                const req = httpMock.expectOne({ method: 'PUT' });
                req.flush(JSON.stringify(returnedFromService));
            });

            it('should return a list of SystemNotification', async () => {
                const returnedFromService = Object.assign(
                    {
                        content: 'BBBBBB',
                        status: 'BBBBBB',
                        sendDate: currentDate.format(DATE_TIME_FORMAT),
                        sourceId: 'BBBBBB',
                        sourceType: 'BBBBBB',
                        sourceName: 'BBBBBB',
                        applicationKey: 'BBBBBB',
                        error: 'BBBBBB',
                        errorShortName: 'BBBBBB',
                        errorDate: currentDate.format(DATE_TIME_FORMAT),
                        deleted: true,
                        deletedDate: currentDate.format(DATE_TIME_FORMAT)
                    },
                    elemDefault
                );
                const expected = Object.assign(
                    {
                        sendDate: currentDate,
                        errorDate: currentDate,
                        deletedDate: currentDate
                    },
                    returnedFromService
                );
                service
                    .query(expected)
                    .pipe(
                        take(1),
                        map(resp => resp.body)
                    )
                    .subscribe(body => expect(body).toContainEqual(expected));
                const req = httpMock.expectOne({ method: 'GET' });
                req.flush(JSON.stringify([returnedFromService]));
                httpMock.verify();
            });

            it('should delete a SystemNotification', async () => {
                const rxPromise = service.delete(123).subscribe(resp => expect(resp.ok));

                const req = httpMock.expectOne({ method: 'DELETE' });
                req.flush({ status: 200 });
            });
        });

        afterEach(() => {
            httpMock.verify();
        });
    });
});
